...
public class CubeFace extends Parent {
  ...
  public CubeFace(CubeModel model, int face) {
    ...
    zPos.addListener(new ChangeListener() {
  ...
  final Rectangle createFaceRectangle() {
    faceRect = RectangleBuilder.create()
      .width(edgeLength)
      .height(edgeLength)
  ...
  Node createMapTiles(int face) {
    if (face == FRONT_FACE ||   
      ...
      ImageView mapTile = ImageViewBuilder.create()
        .image(new Image("http://mt3.google.com/vt/v=w2.97&x=2&y=8&z=4"))
        .fitWidth(CubeFace.edgeLength / 4)
        .fitHeight(CubeFace.edgeLength / 4)
        .build();
      ...
      ImageView mapTile = ImageViewBuilder.create()
        .image(new Image("http://mt3.google.com/vt/v=w2.97&x=2&y=15&z=4"))
        .fitWidth(CubeFace.edgeLength / 4)
        .fitHeight(CubeFace.edgeLength / 4)
        .build();
      mapTile.opacityProperty().bind(cubeModel.mapOpacity);
  ...
  Node createSideMapTiles(int sideNum) {
    ...
    tilePane = TilePaneBuilder.create()
      .prefColumns(2)
      .prefRows(2)
      .build();
    ...
    for (int y = 3; y <= 4; y++) {
      for (int x = xOffset; x <= xOffset + 1; x++) {
        int xm = (x + 1) % 8;
        ImageView mapTile = ImageViewBuilder.create()
          .image(new Image("http://mt3.google.com/vt/v=w2.97&x=" + xm + "&y=" + y + "&z=3"))
          .fitWidth(CubeFace.edgeLength / 2)
          .fitHeight(CubeFace.edgeLength / 2)
          .build();
        mapTile.opacityProperty().bind(cubeModel.mapOpacity);
    ...
    tilePane = TilePaneBuilder.create()
      ...
      .effect(
        PerspectiveTransformBuilder.create()
          .ulx(edgeLength * 0.375)
          .uly(edgeLength * 0.625)
          .urx(edgeLength * 0.625)
          .ury(edgeLength * 0.625)
          .llx(0)
          .lly(edgeLength)
          .lrx(edgeLength)
          .lry(edgeLength)
    ...
    for (int y = 0; y <= 2; y++) {
      for (int x = xOffset; x <= xOffset + 1; x++) {
        ...
        ImageView mapTile = ImageViewBuilder.create()
          .image(new Image("http://mt3.google.com/vt/v=w2.97&x=" + xm + "&y=" + y + "&z=3"))
          .fitWidth(CubeFace.edgeLength / 2)
          .fitHeight(CubeFace.edgeLength / 3)
          .build();
        mapTile.opacityProperty().bind(cubeModel.mapOpacity);
        tilePane.getChildren().add(mapTile);
      }
    ...
    tilePane = TilePaneBuilder.create()
      ...
      .effect(
        PerspectiveTransformBuilder.create()
          ...
          .urx(edgeLength)
          .ury(0)
          .llx(edgeLength * 0.375)
          .lly(edgeLength * 0.375)
          .lrx(edgeLength * 0.625)
          .lry(edgeLength * 0.375)
          .build()
      )
      .build();
  ...
  Paint computeFaceHSB() {
    Paint color = Color.hsb(FACE_HUE, FACE_SAT, Math.abs(-zPos.getValue() / (radius * 2)) + 0.40);
    return color;
  }
}
