...
public class CubeNode extends Parent {
  ...
  public CubeNode(CubeModel model) {
    ...
    angleX.addListener(new ChangeListener() {
    ...
    rearFace.getTransforms().setAll(
      TranslateBuilder.create()
        .x(0)
        .y(0)
        .z(CubeFace.edgeLength)
        .build(),
      RotateBuilder.create()
        .angle(180.0)
        .axis(Rotate.Y_AXIS)
        .pivotX(CubeFace.edgeLength / 2)
        .build()
    );
    ...
    getTransforms().setAll(
      xRotate = RotateBuilder.create()
        .axis(Rotate.X_AXIS)
        .pivotX(CubeFace.edgeLength * 0.5)
        .pivotY(CubeFace.edgeLength * 0.5)
        .pivotZ(CubeFace.edgeLength * 0.5)
        .build(),
      yRotate = RotateBuilder.create()
        .axis(Rotate.Y_AXIS)
        .pivotX(CubeFace.edgeLength * 0.5)
        .pivotY(CubeFace.edgeLength * 0.5)
        .pivotZ(CubeFace.edgeLength * 0.5)
    ...
    setOnMouseDragged(new EventHandler<MouseEvent>() {
      public void handle(MouseEvent me) {
        if (me.isControlDown()) {
          ...
          double proposedTranslateZ = curTranslateZ + ((me.getScreenY() - getScene()
                  .getWindow().getY()) - dragStartOffsetY) * 10;
          ...
          angleY.setValue((((me.getScreenX() - getScene().getWindow().getX()) 
                  - dragStartOffsetX)) / 3 * -1 + dragPressedAngleY);
          angleX.setValue((((me.getScreenY() - getScene().getWindow().getY()) 
                  - dragStartOffsetY)) / 3 + dragPressedAngleX);
        }
      }
    });
  ...
  public void goHomePosition() {
    Timeline homeTimeline = TimelineBuilder.create()
      .keyFrames(
        new KeyFrame(
          new Duration(1000.0),
          new KeyValue(angleX, HOME_ANGLE_X, Interpolator.EASE_BOTH),
  ...
  public Timeline showMapTimeline = TimelineBuilder.create()
    .keyFrames(
      new KeyFrame(
        new Duration(0.0),
        ...
        new KeyValue(cubeModel.mapOpacity, 0.0, Interpolator.LINEAR)
      ),
      new KeyFrame(
        new Duration(1000.0),
        new KeyValue(cubeModel.mapOpacity, 0.7, Interpolator.EASE_BOTH)
      )
    )
    .build();

  public Timeline hideMapTimeline = TimelineBuilder.create()
    .keyFrames(
      new KeyFrame(
        new Duration(0.0),
        new EventHandler<ActionEvent>() {
        ...
        new KeyValue(cubeModel.mapOpacity, 0.7, Interpolator.LINEAR)
      ),
      new KeyFrame(
        new Duration(1000.0),
        new KeyValue(cubeModel.mapOpacity, 0.0, Interpolator.EASE_BOTH)
      )
    )
    .build();
